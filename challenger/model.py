...
class model:
    ...
    def get_varibale(self, dtype=tf.float32):
        ...
        with tf.variable_scope('conv1'):
            self._get_conv_bn_param(filter_size=7, output_channels=32, input_channels=1, W_stddev = 0.0001, isAddW2Loss=True)
        with tf.variable_scope('conv2'):
            self._get_conv_bn_param(filter_size=3, output_channels=32, input_channels=32, W_stddev = 0.0001)
        with tf.variable_scope('conv3'):
            self._get_conv_bn_param(filter_size=3, output_channels=64, input_channels=32, W_stddev = 0.001)
        with tf.variable_scope('conv4'):
            self._get_conv_bn_param(filter_size=3, output_channels=128, input_channels=64, W_stddev = 0.001)
        with tf.variable_scope('conv5'):
            self._get_conv_bn_param(filter_size=1, output_channels=64, input_channels=128, W_stddev = 0.01)
        with tf.variable_scope('conv6'):
            self._get_conv_bn_param(filter_size=3, output_channels=256, input_channels=64, W_stddev = 0.01)
    ...
    def forward(self, X, isTraining):
        ...
        with tf.variable_scope('conv2', reuse=True):
            y = self.conv_bn_relu_pool(y, isTraining, conv_strides=1, conv_padding='VALID', pool_size=3, pool_strides=2, pool_padding='SAME')
        ...
        with tf.variable_scope('conv7', reuse=True):
            # y = self.conv(y, isTraining, conv_strides=1, conv_padding='VALID')
            y = tf.nn.conv2d(y, tf.get_variable('W'), strides=[1,1,1,1], padding='VALID')
        y = tf.reshape(y, shape=[-1,10])
        # with tf.variable_scope('conv1', reuse=True):
        #     P1 = self.conv_bn_relu_pool(X, isTraining, conv_strides=1, conv_padding='VALID', pool_size=2, pool_strides=2, pool_padding='VALID')
        # with tf.variable_scope('conv2', reuse=True):
        #     P2 = self.conv_bn_relu_pool(P1, isTraining, conv_strides=1, conv_padding='VALID', pool_size=2, pool_strides=2, pool_padding='VALID')
        # with tf.variable_scope('conv3', reuse=True):
        #     P3 = self.conv_bn_relu_pool(P2, isTraining, conv_strides=1, conv_padding='VALID', pool_size=2, pool_strides=2, pool_padding='VALID')
        # with tf.variable_scope('conv4', reuse=True):
        #     # P4 = self.conv_bn_relu_pool(P3, isTraining, conv_strides=1, conv_padding='VALID', pool_size=2, pool_strides=2, pool_padding='VALID')
        #     P4 = self.conv_bn_relu(P3, isTraining, conv_strides=1, conv_padding='VALID') 
        # with tf.variable_scope('conv5', reuse=True):
        #     Z5 = tf.nn.conv2d(P4, tf.get_variable('W'), strides=[1,1,1,1], padding='VALID', name='conv')
        # y = tf.reshape(Z5, shape=[-1,10])
        return y
    ...
    def conv_block(self, X, isTraining, strides, name):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('c'):
                y = self.conv_bn(y, isTraining, conv_strides=strides, conv_padding='VALID')
            with tf.variable_scope('d'):
    ...
    def resnet_forward(self, X, isTraining):
        with tf.variable_scope('stage1', reuse=True):
            y = self.conv_bn_relu_pool(X, isTraining, conv_strides=2, conv_padding='SAME', pool_size=2, pool_strides=2, pool_padding='VALID')
        ...
        with tf.variable_scope('stage5', reuse=True):
            ...
            y = self.ID_block(y, isTraining, name='id1')
            y = self.ID_block(y, isTraining, name='id2')            
            y = tf.nn.avg_pool(y, ksize=[1,8,8,1],strides=[1,1,1,1],padding='VALID')
    ...
    def do_it_resnet(self, sess, data, epchos, batch_size, reg, starter_learning_rate, decay_steps, decay_rate):
        ...
        isTraining = True
        Z_train = self.resnet_forward(X_pad, isTraining)
        loss_train = self.loss(Z_train, y, reg)
        accuracy_train = self.accuracy(Z_train, y)

        isTraining = False
        Z_val_test = self.resnet_forward(X_pad, isTraining)
