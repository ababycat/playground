...
class Dense_UNet(nn.Module):
    ...
    def __init__(self, num_classes=1, num_filters=32, pretrained=False, is_deconv=False):
        ...
        :param pretrained:
            False - no pre-trained network is used
            True  - encoder is pre-trained with DenseNet121
        :is_deconv:
            False: bilinear interpolation is used in decoder
            True: deconvolution is used in decoder
        """
...
# model = unet11(num_classes=3).cuda()
# print(model)
# t = torch.randn(3, 512, 512).cuda()
...
class ShallowNet(nn.Module):
    def __init__(self):
        super().__init__()
        kernel_size = 9
        self.conv1 = nn.Conv2d(3, 4, kernel_size, stride=1, padding=int((kernel_size-1)/2), bias=True)
...
#         self.bn1 = nn.BatchNorm2d(4)
#         self.activation1 = nn.ELU()
#         # self.conv2 = nn.Conv2d(4, 1, kernel_size, stride=1, padding=int((kernel_size-1)/2), bias=True)
