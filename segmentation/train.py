...
def train_AlbuNet_LSTM(model, loss_fn, optimizer, loader_train, log, **config):
...
def train_ShallowNet(model, loss_fn, optimizer, loader_train, log, **config):
    ...
    for epoch in range(epochs):
        ...
        for x, y in loader_train:           
            if is_cuda_type:
                x_train = x.type(torch.cuda.FloatTensor)
                y_train = y.type(torch.cuda.FloatTensor)
            else:
                x_train = x.type(torch.FloatTensor)
                y_train = y
                       
            net_output = model(x_train)
            # print(net_output.max().item(), net_output.min().item())
            ...
            if (step+1) % print_every == 1:
                lr = optimizer.param_groups[0]['lr']
                print('lr=%.9f, t = %d, loss = %.9f,' % (lr, step + 1, loss))
