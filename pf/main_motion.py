...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if distance < 0:
            raise ValueError, "distance smaller than zero"
        if abs(steering) > max_steering_angle:
            raise ValueError, "steering bigger than max steering angle"
        ...
        steering = random.gauss(steering, self.bearing_noise)
        distance = random.gauss(distance, self.distance_noise)

        result.orientation = (result.orientation + steering) % (2*pi)
        result.x = 
        result.y = 
