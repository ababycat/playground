...
def preprocess_data(slam_func):
    def wrapper(*args, **kwargs):
        new_args = [x for x in args]
        new_args[0] = new_args[0][0]
        return slam_func(*new_args, **kwargs)
    return wrapper
...
def show_results_online(mu, data):
    x, y = 50, 50
    xy = []
    lxy = []
    xy.append([x, y])
    ...
    mu_np = np.array(mu.value)
    for l, [x, y] in enumerate(mu_np.reshape(-1, 2)):
        circle = Circle((x, y), 1, alpha=1, fill=True, color=cm.rainbow(int(float(l)/num_landmarks*255.0)))
        ax.add_patch(circle)
        # ax.text(x, y, str(l))
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for measure, motion in data:
        # sense
        for l, lx, ly in measure:
            ld = [lx, ly]
            # add 
            for b in range(2):
                Omega.value[b][b] += 1.0 / measurement_noise
                Omega.value[2+l*2 + b][2+l*2 + b] += 1.0 / measurement_noise
                Omega.value[b][2+l*2 + b] -= 1.0 / measurement_noise
                Omega.value[2+l*2 + b][b] -= 1.0 / measurement_noise
                
                Xi.value[b][0] -= ld[b] / measurement_noise
                Xi.value[2+l*2 + b][0] += ld[b] / measurement_noise

        # move
        Omega_expand = Omega.expand(po, po, [0, 1] + list(range(4, po)), [0, 1] + list(range(4, po)))
        Xi_expand = Xi.expand(po, 1, [0, 1] + list(range(4, po)), [0])
        
        for b in range(4):
            Omega_expand.value[b][b] += 1.0 / motion_noise
        for b in range(2):
            Omega_expand.value[b][2+b] -= 1.0 / motion_noise
            Omega_expand.value[2+b][b] -= 1.0 / motion_noise
            Xi_expand.value[b][0] -= 1.0 / motion_noise * motion[b]
            Xi_expand.value[b+2][0] += 1.0 / motion_noise * motion[b]

        A = Omega_expand.take([0, 1], list(range(2, po)))
        A_tr = Omega_expand.take(list(range(2, po)), [0, 1])
        B = Omega_expand.take([0, 1], [0, 1])
        B_inv = B.inverse()
        C = Xi_expand.take([0, 1], [0])
        
        Omega_prime = Omega_expand.take(list(range(2, po)), list(range(2, po)))
        Xi_prime = Xi_expand.take(list(range(2, po)), [0])

        Omega = Omega_prime - A_tr * B_inv * A
        Xi = Xi_prime - A_tr * B_inv * C
...
# show_results(result, data)

# Uncomment the following three lines to run the online_slam routine.

data = make_data(N, num_landmarks, world_size, measurement_range, motion_noise, measurement_noise, distance)
mu, _ = online_slam(data, N, num_landmarks, motion_noise, measurement_noise)
print_result(1, num_landmarks, mu)
show_results_online(mu, data)
