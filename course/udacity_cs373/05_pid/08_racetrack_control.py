...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        r = radius
        if self.x < r:
            cte = r - sqrt((self.x-r)**2 + (self.y-r)**2)
        elif self.x > radius * 3:
            cte = r - sqrt((self.x-r*3)**2 + (self.y-r)**2)
        elif self.y > r:
            cte = 2 * r - self.y
        elif self.y <= r:
            cte = self.y
...
radius = 25.0
params = [10.0, 15.0, 0]
err, tracks_x, tracks_y = run(params, radius, True)
...
r = radius
y1 = np.arange(0, 2*r, 0.1)
x1 = r - np.sqrt(r**2 - (y1-r)**2)

x2 = np.arange(r, 3*r, 0.1)
y2 = np.ones_like(x2) * 2 * r

y3 = np.arange(2*r, 0, -0.1)
x3 = np.sqrt(r**2 - (y3-r)**2) + 3*r

x4 = np.arange(3*r, r, -0.1)
y4 = np.zeros_like(x4)

x = np.hstack((x1, x2, x3, x4))
y = np.hstack((y1, y2, y3, y4))
plt.plot(x, y, label='race')
plt.plot(tracks_x, tracks_y, label='track')
plt.legend()
plt.show()
